...
export default async function CampaignPage({ params }: { params: { slug: string } }) {
  const campaign = await prisma.campaign.findUnique({ where: { slug: params.slug }, include: { organizer: true } })
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8 grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-8">
      <section>
        <div className="rounded-2xl overflow-hidden border border-slate-200 bg-white shadow-sm">
          <div className="aspect-[16/9] bg-slate-100">
            ...
            <img src={campaign.coverPublicUrl || '/logo.png'} alt={campaign.title} className="h-full w-full object-cover" />
          </div>
          <div className="p-5">
            <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900">{campaign.title}</h1>
            <p className="mt-1 text-sm text-slate-600">Category: <span className="font-semibold">{campaign.category}</span></p>
            ...
            <div className="mt-6">
              ...
              <p className="mt-2 text-slate-700">(MVP) Story editor + images + YouTube embeds will be added next.</p>
            </div>

            <div className="mt-6">
              <h2 className="text-lg font-extrabold">Updates</h2>
              <p className="mt-2 text-slate-700">(MVP) Story updates + attachments will be added next.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="lg:sticky lg:top-24 h-fit space-y-4">
        <DonateCard campaignId={campaign.id} connectedAccountId={campaign.organizer.stripeAccountId} currency={campaign.currency} />
